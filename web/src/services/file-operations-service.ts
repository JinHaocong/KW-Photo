...
/**
 * Moves selected files to a target folder using the real file path edit API.
...
/**
 * Creates a file share link from selected files.
 * @param options Authenticated API client options.
 * @param params Share options and selected file ids.
 * @returns Created share link metadata.
 */
export const createFileShareLink = (options: ApiClientOptions, params: CreateFileShareParams): Promise<FileShareLinkResponse> => {
...
/**
 * Fetches the user's album list for the preview add-to-album action.
 * @param options Authenticated API client options.
...
/**
 * Ensures the official favorites album exists and returns its file membership.
 * @param options Authenticated API client options.
 * @returns Favorites album metadata and file ids.
 */
